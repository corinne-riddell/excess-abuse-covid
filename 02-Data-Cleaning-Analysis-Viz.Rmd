...
This analysis was inspired by a Buzzfeed analysis of excess deaths and their published R code, which is available here: https://github.com/BuzzFeedNews/2021-05-tx-winter-storm-deaths
...
This code assumes and R project directory with the following sub-folders:
...
* "CM"  - first-hand account child maltreatment or abuse
* "DV"  - child-witness intimate partner violence (also known as domestic violence)
* "IPV" - first-hand account intimate partner violence
...
Weekly.CM <- read.csv("WeeklyData/US-Weekly-CM.csv", stringsAsFactors = F) %>% 
...
Weekly.DV <- read.csv("WeeklyData/US-Weekly-DV.csv", stringsAsFactors = F) %>% 
  pivot_longer(cols = c("sample_0":"sample_9"), names_to = "SampleNo" , values_to = "value") %>% 
  mutate(date = as.Date(period, format = "%Y-%m-%d")) %>% 
  group_by(date) %>% 
  summarise(mean_value = mean(value)) %>% 
...
Weekly.IPV <- read.csv("WeeklyData/US-Weekly-IPV.csv", stringsAsFactors = F) %>% 
  pivot_longer(cols = c("sample_0":"sample_9"), names_to = "SampleNo" , values_to = "value") %>% 
  mutate(date = as.Date(period, format = "%Y-%m-%d")) %>% 
  group_by(date) %>% 
  summarise(mean_value = mean(value)) %>% 
...
Monthly.CM <- read.csv("MonthlyData/US-Monthly-CM.csv", stringsAsFactors = F)%>% 
...
Monthly.DV <- read.csv("MonthlyData/US-Monthly-DV.csv", stringsAsFactors = F)%>% 
  mutate(date = as.Date(timestamp, format = "%Y-%m-%d"),
         month_of_year = month(date),
         mean_value = value,
         abuse_type = "DV") %>% 
...
Monthly.IPV <- read.csv("MonthlyData/US-Monthly-IPV.csv", stringsAsFactors = F) %>% 
  mutate(date = as.Date(timestamp, format = "%Y-%m-%d"),
         month_of_year = month(date),
         mean_value = value,
         abuse_type = "IPV") %>% 
...
for (i in unique(Weekly.Data$abuse_type)) {
  ...
  Weekly_Excess_Searches <- bind_rows(Weekly_Excess_Searches,tmp.w)
  
  Abuse.i.Monthly <- bind_cols(Abuse.i.Monthly, Monthly_pred)
  Monthly_Excess_Searches <- bind_rows(Monthly_Excess_Searches,tmp.m)
...
CM.Weekly.plot <- ggplot(Weekly_Excess_Searches %>% filter(abuse_type == "CM"), 
  ...
  labs(title = "(A) first-hand account child abuse") +
...
IPV.Weekly.plot <- ggplot(Weekly_Excess_Searches %>% filter(abuse_type == "IPV"), 
  ...
  theme_minimal(base_size = 16) +
  xlab("") + ylab("Search Outcome") +
  labs(title = "(C) first-hand account intimate partner violence") +
  theme(legend.position = "none",
